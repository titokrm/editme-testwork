...
const config = {
  ...
  if (needCompil) {
    return gulp.src('./'+sourcesDir+'/images/svgIcons/*.svg')
      .pipe(cheerio({
          run: function($) {
...
function copyDir(done) {
  if (needCompil) {
  gulp.src('./'+sourcesDir+'/phpmailer/**/*')
      .pipe(gulp.dest(resultDir+'/phpmailer/'));
  }
  done();
}
...
function watchFiles(done) {
  if (needCompil) {
    ...
    gulp.watch('./'+sourcesDir+'/images/**/*', gulp.series(copyImg, createSvgSprite, browerReload));
...
gulp.task('copyFiles', gulp.series(copyHtml, copyPhp, copyDir, copyCss, copyFonts, copyJs, copyImg, createSvgSprite, copyFavicon));
...
gulp.task('createSvgIcons', gulp.series(createSvgSprite));